...
import com.oty.util.Util; 

@Controller
@RequestMapping("/onenet")
public class OneNetAction {

	public static final String object_id_generic_sensor = "3300";
	public static final String object_id_set_point = "3308";
	public static final String object_id_digital_output = "3201";
	public static final String object_id_addressable_text_display = "3341";

	private static Logger logger = LoggerFactory.getLogger(OneNetAction.class);
	private static String token = "";// 用户自定义token和OneNet第三方平台配置里的token一致
	private static String aeskey = "";// aeskey和OneNet第三方平台配置里的token一致
	...
	public String receive(@RequestBody String body)
		 ...
		 * logger.info("data receive:  body Object --- " + obj); if (obj !=
		 * null) { boolean dataRight = Util.checkSignature(obj, token); if
		 * (dataRight) { logger.info("data receive: content" + obj.toString());
		 * } else { logger.info("data receive: signature error"); }
		 * 
		 * } else { logger.info("data receive: body empty error"); }
		 */
		/************* 明文模式 end ****************/
