...
import com.oty.sys.model.BaseModel;
...
public class Pager {
	...
	public int getTotalPages() {
		...
		totalPages = getTotalSize() % getPageSize() == 0 ? (getTotalSize() / getPageSize())
				: (getTotalSize() / getPageSize() + 1);
		return totalPages;
	}
	 ...
	 * post提交分页
	 * 
	 * @return <div class="dataTables_info" id="dataTable1_info">当前显示 1 到 10 条，共
	 *         14 条记录</div>
	 *         <div class="dataTables_paginate paging_full_numbers" >
	 *         <a class="first paginate_button paginate_button_disabled"
	 *         >首页</a><a class="previous paginate_button
	 *         paginate_button_disabled" >上一页</a><span><a class=
	 *         "paginate_active">1</a><a class="paginate_button">2</a></span><a
	 *         class="next paginate_button" >下一页</a><a class="last
	 *         paginate_button" >末页</a> </div>
	 * 
	 */
	public String getShowPage() {
		...
		if (this.getTotalPages() > 1) { // 如果总页数大于一页，才显示分页栏
			...
			if (this.getPageNo() > 1) { // 如果当前页不是第一页，则显示上一页
				sb.append(
						"<a class='first paginate_button'  href='javascript:Pager.gotoPage( 1)'>首页</a><a   class='previous paginate_button'  href='javascript:Pager.gotoPage( "
								+ (this.getPageNo() - 1) + ")'>上一页</a>");
			} else {
				sb.append(
						"<a class='first paginate_button paginate_button_disabled'  >首页</a><a   class='previous paginate_button paginate_button_disabled' >上一页</a>");
			}

			if (totalPages > 1) {
				int beginNum = pageNo - 2;
				if (beginNum > totalPages - 4)
					beginNum = totalPages - 4;
				if (beginNum < 1)
					beginNum = 1;
				...
				for (int i = beginNum; i <= endNum; i++) {
					if (i == totalPages) {
						sb.append("<a  class='" + ((pageNo == i) ? "paginate_active" : "paginate_button")
								+ "'  href='javascript:Pager.gotoPage(" + i + ")' >" + i + "</a>");
					} else {
						sb.append("<a   	class='" + ((pageNo == i) ? "paginate_active" : "paginate_button")
								+ "' href='javascript:Pager.gotoPage(" + i + ")' >" + i + "</a>");
					}
			...
			if (this.getPageNo() < totalPages) { // 如果当前页不是最后一页，则显示下一页
				sb.append("	<a  class='next paginate_button'   href='javascript:Pager.gotoPage("
						+ (this.getPageNo() + 1)
						+ ")' >下一页</a><a   class='last paginate_button'   href='javascript:Pager.gotoPage("
						+ getTotalPages()
						+ ")' 	 >末页</a><input  id='pgsize'   value=''  style='margin-left:5px;width:20px' /><a  class='next paginate_button'   href='javascript:var tosize="
						+ totalPages
						+ ";var size=$(\"#pgsize\").val();if(size>tosize){size=tosize};Pager.gotoPage(size);' >跳到</a> ");
			} else {
				sb.append(
						"<a  class='first paginate_button paginate_button_disabled'  >下一页</a><a   class='previous paginate_button paginate_button_disabled' >末页</a><input  id='pgsize'   value=''  style='margin-left:5px;width:20px' /><a  class='next paginate_button'   href='javascript:var tosize="
								+ totalPages
								+ ";var size=$(\"#pgsize\").val();if(size>tosize){size=tosize};Pager.gotoPage(size);' >跳到</a> ");
			}
			...
			showPage = showPage.replace("{0}", String.valueOf(getBegin())).replace("{1}", String.valueOf(getEnd()))
					.replace("{2}", String.valueOf(getTotalSize()));
			return showPage;
		}
		return "";
	}
