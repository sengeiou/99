	...
	public static String doPost(String url, JSONObject json){
		...
		try {
			...
			if(res.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
				...
				//调用微信小程序接口 返回图片二进制内容  写入 生成图片 返回路径
				...
				InputStream inputStreamUrl = new FileInputStream(file);
				String key = OssUtils.getKey(UUID.randomUUID().toString(), file.getName());
				new OssUtils().uploadFile(inputStreamUrl, key);
				result = OssUtils.getUrl(key);
			}
		} catch (Exception e) {
