...
public class Session implements Serializable {
	...
	private Map<String, Session> sessions = new HashMap<String, Session>(); // 用于dwr
																			// websocket存储多开页面创建的session
	...
	public boolean write(Object msg) {
		...
		if (session != null && isConnected()) {
			...
			try {
				...
				/*
				 * MessageProto.Model msgModel = (MessageProto.Model)msg;
				 * ScriptBuffer script = new ScriptBuffer(); Object json =
				 * JSONArray.toJSON(msgModel.toByteArray());
				 * script.appendCall(Constants
				 * .DWRConfig.DWR_SCRIPT_FUNCTIONNAME, json);
				 * dwrsession.addScript(script);
				 */
				return true;
			} catch (Exception e) {
