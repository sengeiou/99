package com.oty.server.connertor.impl;
...
import com.oty.server.exception.PushException;
import com.oty.server.group.ImChannelGroup;
...
import com.oty.server.session.impl.SessionManagerImpl;
import com.oty.web.dwrmanage.DwrUtil;
...
public class ImConnertorImpl implements ImConnertor {
	...
	public void pushMessage(MessageWrapper wrapper) throws RuntimeException {
		try {
			...
			/*
			 * 服务器集群时，可以在此
			 * 判断当前session是否连接于本台服务器，如果是，继续往下走，如果不是，将此消息发往当前session连接的服务器并
			 * return
			 * if(session!=null&&!session.isLocalhost()){//判断当前session是否连接于本台服务器
			 * ，如不是 //发往目标服务器处理 return; }
			 */
			if (session != null) {
	...
	public void connect(ChannelHandlerContext ctx, MessageWrapper wrapper) {
		try {
			...
			if (StringUtils.isNotEmpty(sessionId0) || sessionId.equals(sessionId0)) {
				...
				log.info("connector connect sessionId -> " + sessionId + ", sessionId0 -> " + sessionId0 + ", ctx -> "
						+ ctx.toString());
